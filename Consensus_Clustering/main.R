...
ConsensusClusterPlus <- function( d=NULL,
  ...
  # 
  # #Gray shades colors for marking different clusters
  # thisPal <- c("#B2BEB5", #Ash Gray
  #              "#A9A9A9", #Dark Gray
  #              "#808080", #Gray
  #              "#D3D3D3", #Light Gray
  #              "#E5E4E2", #Platinum
  ...
  #my colors for marking different clusters
  thisPal <- c("#8bd3c7", 
               "#ebdc78", 
               "#b3d4ff",
               "#8be04e") 
...
#     
    if(!is.null(plot) && plot=="pngBMP"){
      ...
      heatmap(pc, Colv = NA, Rowv = NA, symm = FALSE, scale = "none", col = tmyPal, na.rm = TRUE, labRow = F, labCol = F, mar = c(5, 5), main = paste("Consensus Clustering (k=",tk,")", sep = ")"), ColSideCol = oc)
    }else{
      pc = rbind(pc,0)
      #former with tree:
      heatmap(pc, Colv=as.dendrogram(hc), Rowv=NA, symm=FALSE, scale='none', col=tmyPal, na.rm=TRUE,labRow=F,labCol=F,mar=c(5,5),main=paste("Consensus Clustering (k=",tk,")",sep="") , ColSideCol=colorList[[1]])
    }
    # legend("topright", inset=c(-0.05, 0.1),legend= c('cluster 1','cluster 2','cluster 3','cluster 4','cluster 5','cluster 6'),
    #        fill=unique(colorList[[1]]),horiz=FALSE)
...
# 
# 
#gbm <- read.table("before_cc_vals.txt",header = FALSE)
gbm <- read.table("cluster_values_aa.txt",header = FALSE)
...
tmyPal=c("#ffffff", "#e6eaed", "#ced6dc","#b6c1ca", "#9dacb8", "#8498a6",
         "#6c8395","#546e83","#3b5971", "#224560","#0a304e")
# 
# 
results = ConsensusClusterPlus(gbm,maxK=4,writeTable=TRUE,reps=1000,pItem=.8,pFeature=1,title=title,
                               clusterAlg="hc",distance="pearson",seed=1262118388.71279,plot="png",tmyPal=tmyPal)
# 
RECTALCLSS <- read.table("cluster_labels_aa.txt",header = FALSE) # load class labels
...
write.table(labels, "output_values.txt", sep="\t",col.names = F, row.names = F)
