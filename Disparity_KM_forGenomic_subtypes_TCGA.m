...
save_folder=[folder, '/genomic_KM_TCGA/'];
...
for endpoint = 1:3
    for population= 1:3
        ...
        newtbl=readtable([folder,'KM_Curves_genomic_TCGA.xlsx']);
        ...
        switch endpoint
            case 1
                code_word='Disease-specific survival probability';
                %%Disease-specific Survival [month] (0:ALIVE OR DEAD TUMOR FREE) (1:DEAD WITH TUMOR)
                timeToEvent=( table2array(newtbl(:,3)));
                status=table2array(newtbl(:,4));
            case 2
                code_word='Progression-free survival probability';
                %%Progression-free Survival [months](0:CENSORED, 1:PROGRESSION)
                timeToEvent=( table2array(newtbl(:,7)));
                status=table2array(newtbl(:,8));
            case 3
                ...
                timeToEvent=round(timeToEvent/30);
                status=table2array(newtbl(:,6));
        end
        
        
        risk_category=table2array(newtbl(:,9));
